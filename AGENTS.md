...
- Batch reads such as `get_problem_batch` are allowed during an actual run or explicit post-implementation validation after the profile and accepted grammar are frozen. Batch results must be fed independently through that existing parser; they must not be used to broaden or redesign the profile on the fly. One rejected record gets zero writes and does not block the remaining records.
